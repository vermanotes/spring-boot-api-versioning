...
/**
 * VersionedRequestMappingHandlerMapping class
 *
...
public class VersionedRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
	...
	protected RequestMappingInfo createRequestMappingInfo(RequestMapping requestMapping,
		...
		if (customCondition instanceof ApiVersionRequestCondition) {
			return RequestMappingInfo
					.paths(Arrays.stream(requestMapping.path())
					...
					.mappingName(requestMapping.name()).customCondition(customCondition).build();
		}
	 ...
	 * @return
	 */
	private Set<Range<VersionData>> createVersionRangeSet(@Nonnull ApiVersion apiVersion) {
		final String[] supported = apiVersion.value();

		if (ArrayUtils.isNotEmpty(supported)) {
			return Arrays.stream(supported).map(VersionData::parse).map(Range::singleton).collect(Collectors.toSet());
		}
		...
		final BoundType lowerBoundType = StringUtils.isNotBlank(atMost) ? BoundType.CLOSED
				: StringUtils.isNotBlank(greaterThan) ? BoundType.OPEN : null;

		final BoundType upperBoundType = StringUtils.isNotBlank(atLeast) ? BoundType.CLOSED
				: StringUtils.isNotBlank(lessThan) ? BoundType.OPEN : null;
		final String lowerVersion = StringUtils.isNotBlank(atMost) ? atMost
				: StringUtils.isNotBlank(greaterThan) ? greaterThan : null;
		final String upperVersion = StringUtils.isNotBlank(atLeast) ? atLeast
				: StringUtils.isNotBlank(lessThan) ? lessThan : null;

		final Range<VersionData> versionRange = (StringUtils.isNotBlank(lowerVersion)
				&& StringUtils.isNotBlank(upperVersion))
						? Range.range(VersionData.parse(lowerVersion), lowerBoundType, VersionData.parse(upperVersion),
								upperBoundType)
						: StringUtils.isNotBlank(lowerVersion)
								? Range.downTo(VersionData.parse(lowerVersion), lowerBoundType)
								: StringUtils.isNotBlank(upperVersion)
										? Range.upTo(VersionData.parse(upperVersion), upperBoundType)
										: null;
